...
def run_category(model, temperature, num_predict, category_list,  prompt):
    llm = ChatOllama(model=model, temperature=temperature, num_predict=num_predict)
    ctg = CategoryFactory(category_list).create_category_model()
    structured_llm = llm.with_structured_output(ctg, method="json_schema")
...
def run_processor(model, temperature, num_predict, prompt):
    llm = ChatOllama(model=model, temperature=temperature, num_predict=num_predict)
    ...
    res = user_defined_runnables.invoke(prompt)
    return res

# ~ Func 5
# Define a func to do the fuzzy_label_checker job
def match_fuzzy_llm(model, temperature, num_predict,prompt):
    llm = ChatOllama(model=model, temperature=temperature, num_predict=num_predict)
    user_defined_runnables =  llm | RunnableParallel({
        "JOutput": JsonOutputParser(),
        "Duration": RunnableLambda(lambda output: output.response_metadata['total_duration']/1e9)
    })
    return user_defined_runnables.invoke(prompt)
...
if __name__ == "__main__":
    ...
    print(run_category(model, temperature, num_predict, category_list, prompt))
...
"""
    ...
    res = run_processor(model, temperature, num_predict, prompt)
    print(res)
    ...
    res = match_fuzzy_llm(model, temperature, num_predict, prompt)
    print(res)